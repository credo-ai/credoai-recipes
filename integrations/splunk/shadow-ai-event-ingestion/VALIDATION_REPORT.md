...
| `401` mid-run (token goes stale *after* a successful exchange) | Not triggered — requires a real token expiry mid-run, not reproducible on demand. Code path (refresh once, retry, fatal on second `401`) reviewed but not exercised live. |
